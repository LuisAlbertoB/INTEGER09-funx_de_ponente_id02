...
// GET /api/eventos -> Listar eventos (paginado y con filtros)
...
// GET /api/eventos/:id -> Obtener detalle de evento (público para autenticados)
...
// GET /api/eventos/recomendados -> Obtener recomendaciones personalizadas
router.get('/recomendados', async (req, res) => {
  try {
    const recomendaciones = await eventosService.getRecomendaciones(req.user.id_usuario);
...
// POST /api/eventos/:id/feedback-asistente -> Enviar telemetría de engagement
router.post('/:id/feedback-asistente', async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({ message: 'Feedback de telemetría guardado.', feedback });
  } catch (error) {
    return res.status(error.statusCode || 500).json({ message: error.message });
  }
});
